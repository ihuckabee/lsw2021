...
eqw=eqw['ew'].to_numpy()
#eqw peaks
maxima = find_peaks(eqw_ew_lim, prominence = 0.1)
...
for i in range(len(min_pos)):
    eqw_temp = np.asarray(np.where(np.logical_and(max_pos < min_pos[i]+0.015,max_pos > min_pos[i]-0.015)))
    #eqw_temp = np.asarray(np.where(np.logical_and(max_pos < min_pos[i]+min_wid[i], max_pos > min_pos[i]-min_wid[i])))
...
for i in range(len(eqw_pos_merged)):
   plt.vlines(max_pos[eqw_pos_merged[i]], 0, 1, color='green', alpha = 0.5) 
toc()
pdb.set_trace()