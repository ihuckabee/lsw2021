...
def read_eqw_file(eqw_file: str) -> pd.DataFrame:
  ...
  #drop col 13, 14, 15 for data with comments - otherwise, # this line out 
...
def write_pairs_line_list(line_list: Dict, output_path: str):
  ...
  pairs = list(permutations(lines, 2)) #***changed from pairs = permutations(lines,2) to pairs = list(permutations(lines,2))
  print(len(lines))
  print(len(pairs))

  # Edit wavelengths
  start_wavelength = 3000  # angstroms
  max_wavelength = 3500  # angstroms
  wavelength_step = 2.5  # between each line
  current_wavelength = start_wavelength
  oscillator = 1 
  output_list = [[] for i in range(500)] #creates 10 linelists

  for output_lines in output_list: 
    for pair in pairs:
      for line in pair:
        # Random shift in wavelength
        # current_wavelength += np.random.uniform(-1, 1)
        current_wavelength = start_wavelength + wavelength_step*(np.random.random())
        new_line = copy(line)
        new_line.wavelength = current_wavelength
        output_lines.append(new_line)
        oscillator = -1*oscillator
        if oscillator == 1:
          start_wavelength += wavelength_step
      if current_wavelength >= max_wavelength-wavelength_step:
  ...
  # Create LineCollection for each key
    ...
    with open(output_path+str(output_list.index(output_lines))+output_ending, 'w', encoding='utf-8') as outfile:
...
if __name__ == "__main__":
  res_dir = "res" #***changed from '../res' to just res and same for out: this script is one dir above res and out 
  out_dir = "out"
  identifier = "vald-4800-5300-for-grid"
  line_file = f"{res_dir}/{identifier}.list"
  spec_file = f"{res_dir}/{identifier}_spec.txt" #***in my cases i turned my spec/eqws into .txt files 
  eqw_file = f"{res_dir}/{identifier}_eqw.txt"
  ...
  output_file = f"{out_dir}/{identifier}"
  output_ending = "-paired.list"
